...
- 例:`stages/get_pr_sigma.py` 当前是 `subprocess` 调 `2-data_process/get_PR/Sigma/check_sigma.py`,后续 PR 才会原生实现。
...
├── Combine_data/           # calculate.py 是新包 fmc_combine_data 的 byte-equal 参考
...
### 2.3 Byte-identical compatibility(核心 invariant)

**新包的输出必须与 legacy 脚本字节级一致**——这是迁移期的硬约束:

- 验证工具:`scripts/compare_fmc_csv_byte_equal.py`、`scripts/compare_mc_csv_byte_equal.py`
- Test fixture 中的 `expected/*.csv` **必须由 legacy 脚本生成**,绝不手写或用新代码生成
- 见 `tests/fixtures/fmc_combine_data/README.md` 的生成流程
- `stages/fmc_combine_data.py` 顶部 docstring 明确写出 byte-identical 的 Python 版本前提(3.9+, csv 默认 excel dialect, `str(float)` 表达)

**Claude Code 规则**:涉及 stage 实现或输出格式时,任何改动前先思考"会不会破坏 byte-equal";有疑问就先 stop 问用户。
...
- **golden / expected 文件不可手写**:byte-equal 测试的 `expected/*.csv` 必须用 legacy 脚本生成,见 `tests/fixtures/fmc_combine_data/README.md`
- 运行:
  ```bash
  pytest tests/
  # EDA 环境无 PyPI 时:
...
- `1-Parse/` 和 `2-data_process/` 下的所有 legacy 脚本——它们是 byte-equal 参考标准
- `2-data_process/get_PR/` 下的 `.rpt` 文件——byte-equal 测试需要它们做 input
...
- ❌ 修改 stage 输出格式时跳过 byte-equal 验证
- ❌ 手写 test fixture 中的 expected CSV(必须用 legacy 脚本生成)
