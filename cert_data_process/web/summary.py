...
def _metric_value(metric: str, basis: str, sig: Optional[dict], mom: Optional[dict]) -> Optional[float]:
    ...
    # lW2 (fallback lW1 when no abs_tol set). Every other metric ignores W2 (-> W1).
    if basis == "w2":
        if metric == "Late_Sigma" and "lW2" in row:
            v = row.get("lW2")
            return v if v is not None else row.get(w1_key)
        return row.get(w1_key)
...
def build_batch(config, stage_execution: list[dict[str, Any]], *, batch_id: str, name: str, when_utc: str) -> dict:
    ...
    return {
        ...
        "config": {
            ...
            "rc_type": getattr(config, "rc_type", ""),
            "library_type": getattr(config, "library_type", "auto"),
            "lib_unit": getattr(config, "lib_unit", ""),
            "fmc_unit": getattr(config, "fmc_unit", ""),
