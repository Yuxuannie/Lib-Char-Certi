...
def _parse_fmc_line(line: str, result: dict[str, float]) -> None:
    """Parse one line using legacy-matching token checks.

    This intentionally mirrors legacy `calculate.py` string matching semantics
    (including trailing-space tokens like ``"Mean   "``) so that meanshift/std/
    skew fields stay byte-compatible with the legacy extraction behavior.
    """
